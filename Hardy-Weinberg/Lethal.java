// Main for Lethal Hardy Weinberg
...
public class Lethal{
    ...
    public static void main(String [] args){
        ...
        System.out.println("population " + ((recCount + domCount)/2));
    }
    ...
    public static ArrayList <Organism> nextGen(ArrayList <Organism> list, int numGens){
        ...
        while((domCount + recCount)/2 != initPop){
            int temp1 = (int)(2*Math.random());
            int temp2 = (int)(2*Math.random());
            int temp3 = (int)(2*Math.random());
            boolean d1, d2, d3;
            ...
            newList.add(new Organism(d1, d2, d3));
            for(int k = newList.size() - 1; k >= 0; k--){
                if(newList.get(k).getGene1() == false && newList.get(k).getGene2() == false){
                    newList.remove(k);
                    recCount = recCount - 2;
                }
            }
